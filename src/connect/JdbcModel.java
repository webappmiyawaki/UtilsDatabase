...
public class JdbcModel implements JdbcCRUD{
    ...
    public Map<String,SampleObject> selectAll() {
        String sql= "SELECT * FROM connect_sample01 ORDER BY id";
        try(Connection conn = JdbcUtils.getConnection();
    ...
    public SampleObject selectUnit(String strNum) {
        ...
        try {
            ...
            try {
                System.out.println("beforeClose pstm:"+pstm.isClosed());
                System.out.println("beforeClose conn:"+conn.isClosed());
                System.out.println("beforeClose rs:"+rs.isClosed());
            } catch (SQLException e) {
                e.printStackTrace();
            }
            JdbcUtils.close(pstm, conn, rs);
            try {
                System.out.println("afterClose pstm:"+pstm.isClosed());
                System.out.println("afterClose conn:"+conn.isClosed());
                System.out.println("afterClose rs:"+rs.isClosed());
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
